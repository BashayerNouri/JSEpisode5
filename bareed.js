...
class Customer extends Person {
  ...
  _isInRange(vendor) {
    let distanition = this.location.distanceTo(vendor.location);
    return distanition <= vendor.range;
  }

  _haveEnoughMoney(vendor, numberOfIceCreams) {
    let cost2 = vendor.price * numberOfIceCreams;
    return cost2 <= this.wallet.money;
  }
